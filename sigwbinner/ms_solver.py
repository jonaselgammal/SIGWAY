...
from sigwbinner.units import \
    efolds_from_wavenumber_si_units, H_from_wavenumber,\
...
def _run_background(Ud, max_efolds, phi0, pvalues, solver_opts):
    ...
    # Initial conditions
    Ud0 = Ud(phi0, *pvalues)
    y0 = -(3 / Ud0) * (jnp.sqrt(1 + 2 / 3 * Ud0**2) - 1)  # Initial derivative wrt N of the rescaled inflaton
    h0 = (1 / jnp.sqrt(6) * (jnp.sqrt(1 + 2 / 3 * Ud0**2) + 1) ** (1 / 2))  # Initial rescaled Hubble rate
...
def _solve_perturbations(Ud, Udd, phiIn, yIn, hIn, nin, nout, lograt, pvalues, solver_opts):
    """
    ...
    dPhiRpRin = -1.0  # Initial value of the derivative of the real part of tilde Delta phi with respect to N
...
def _run_perturbations(Ud, Udd, phiIn, yIn, yOut, hIn, nin, nout, lograt, pvalues, solver_opts):
    """
    ...
    sol = _solve_perturbations(Ud, Udd, phiIn, yIn, hIn, nin, nout, lograt, pvalues, solver_opts)
...
SolverOptions = namedtuple('SolverOptions', ['rtol', 'atol', 'max_steps', 'dt0', 'saveat'])
...
class SingleFieldSolver:
    ...
    """
    def __init__(self, V, phi0=0., pi0=0., N_CMB_to_end=65., max_efolds=1000., 
                 cmb_bounds={}, check_consistency=False,
                 N_subhorizon=3., N_suphorizon=7.,
                 k=None, upsample=False,
                 background_solver_opts={}, perturbation_solver_opts={},
                 error_on_fail=False):
        ...
        self.U = lambda phi, *p: self.V(phi, *p) / self.V(self.phi0, *p) # Normalize to 1 at phi0
        ...
        self.cmb_means = cmb_bounds.get(
            "means",
            jnp.array([3.04442188, 0.96488871, 0.])
        )
        self.cmb_cov = cmb_bounds.get(
            "cov",
            jnp.array([[2.00112315e-04, 1.35106101e-05, 0.],
                       [1.35106101e-05, 1.72537423e-05, 0.],
                       [0.,             0.,           0.01]])
        )
        ...
        if k is None: # If k is None, no upsampling
            ...
            self.k = k
            if not upsample:
                Warning("Providing k and not upsampling will result in k being ignored.")
        elif isinstance(k, jnp.ndarray): # If f is an iterable, convert to array
            self.k = jnp.array(k)
            if not upsample:
                Warning("Providing k and not upsampling will result in k being ignored.")
        else:
            raise ValueError("k should be None, a callable or an iterable. Got {}".format(type(k)))
        ...
        self.background_solver_opts = default_solver_opts['background']._replace(**background_solver_opts)
        invalid_opts = set(self.background_solver_opts._fields) - set(default_solver_opts['background']._fields)
        if invalid_opts:
            raise ValueError(f"Invalid options found in background_solver_opts: {', '.join(invalid_opts)}")
        
        # Set perturbation solver options
        self.perturbation_solver_opts = default_solver_opts['perturbation']._replace(**perturbation_solver_opts)
        invalid_opts = set(self.perturbation_solver_opts._fields) - set(default_solver_opts['perturbation']._fields)
        if invalid_opts:
            raise ValueError(f"Invalid options found in perturbation_solver_opts: {', '.join(invalid_opts)}")
    ...
    def run_background(self, params):
        ...
        """
        bsol = _run_background(self.Ud, self.max_efolds, self.phi0, params, self.background_solver_opts)
    ...
    def run_perturbations(self, k, N, phi, y, h, params):
        ...
        compute_Pzeta = jax.vmap(_run_perturbations, in_axes=(None, None, 0, 0, 0, 0, 0, 0, 0, None, None), out_axes=0)
        Pzetas = compute_Pzeta(self.Ud, self.Udd, phiIn, yIn, yOut, hIn, Nin, Nout, lograt, params, self.perturbation_solver_opts)
    ...
    def run_single_k(self, k, N, phi, y, h, params):
        ...
        # Compute the perturbations with vmap
        sol = _solve_perturbations(self.Ud, self.Udd, phiIn, yIn, hIn, Nin, Nout, lograt, params, solver_opts)
        return sol, lograt
    ...
    def run(self, k, *params):
        ...
        def P_zeta_interpolation(knew, *params):
            ...
            return jnp.interp(knew, k, P_zeta, left=0.,right=0.)
    ...
    def plot_evolution(self, k, params):
        ...
        N_CMB = jnp.max(N) - self.N_CMB_to_end
        H_CMB = jnp.interp(N_CMB, N, h)
        Nk = efolds_from_wavenumber_si_units(k, H_from_wavenumber(k, N, h, N_CMB, H_CMB), N_CMB, H_CMB)
        ...
        for ax in axs:
            ax.axvline(N_CMB-Nend, color='k', linestyle='--', label='CMB scale', alpha=0.5)
...
if __name__ == "__main__":
    ...
    phi0 = 3.0
    pi0 = 0.0
    pvalues = jnp.array([1/jnp.sqrt(2) * (1+0.56*1e-2),1.86e-6*(1-0.12),0.19669*(1+1e-3),0.3*6.23*1e-5])

    robbiesmodel = SingleFieldSolver(
        V, 
        phi0=phi0, 
        pi0=pi0)
    k = jnp.geomspace(1e-5, 10**(-2.), 100)

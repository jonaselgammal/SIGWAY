...
def upsample_f(f, fk, Omega_GW):
    return 10.**(jnp.interp(f, fk, jnp.log10(Omega_GW), left=-30., right=-30.))
...
class Binned_P_zeta:
    def __init__(self, model_name, model_label, nbins=50,
                 path_to_C=None, norm="RD", backend='jax'):
        """
        ...
        if path_to_C is None:
            try:
                data = np.loadtxt(os.path.join(
                    os.path.dirname(__file__), "binning_coefficients", f"tabkpp{nbins}.txt"))
            except FileExistsError:
                raise FileNotFoundError(f"No coefficients found for nbins={nbins}. Please change the number to "
                                        "one where the coefficients have been precomputed for.")
        else:
            try:
                data = np.loadtxt(path_to_C)
            except FileNotFoundError:
                raise FileNotFoundError(f"No coefficients found in {path_to_C}. Your specified path_to_C seems to be incorrect.")
    ...
    def dtemplate_default(self, index, fvec, *A):
        ...
        return self.norm(kvec) * upsample_f_linear(kvec, self.fk, domega) # To avoid log(0)

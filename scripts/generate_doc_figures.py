...
Figures land in docs/assets/images/figures/{name}_{light,dark}.svg and are
embedded in the markdown example pages with Material's #only-light /
#only-dark suffixes.
"""
...
def render(plotfn, base):
    for theme, cfg in THEMES.items():
        rc: dict[RcKeyType, Any] = {
            ...
            "axes.edgecolor": cfg["fg"],
            "axes.labelcolor": cfg["fg"],
            "xtick.color": cfg["fg"],
            "ytick.color": cfg["fg"],
            "text.color": cfg["fg"],
            "axes.spines.top": False,
            "axes.spines.right": False,
            "axes.prop_cycle": cycler(color=cfg["cycle"]),
            "font.size": 12,
        }
...
def pzeta_lognormal(k, logAs, logDelta, logks):
    ...
    return (
        As
        / (jnp.sqrt(2 * jnp.pi) * Delta)
        * jnp.exp(-0.5 / Delta**2 * jnp.log(k / ks) ** 2)
    )
...
t = jnp.concatenate(
    [jnp.linspace(1e-5, 0.999, 200), jnp.geomspace(1.0, 1e3, 800)]
)
model = OmegaGW(
...
render(
    lambda ax: (
        ax.loglog(f, omega),
        ax.set_xlabel("$f$ [Hz]"),
        ax.set_ylabel(r"$\Omega_{\mathrm{GW}}\,h^2$"),
    ),
    "simple_omega",
)
...
amps = [-4.0] * len(binned.parameter_names)
amps[50] = 0.0
